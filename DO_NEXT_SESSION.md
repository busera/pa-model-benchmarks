...
Execute **MB-003 — Synthetic local tool-live execution** as the sole active outcome. Exercise the existing synthetic tool-live gate against the selected local candidate in its isolated localhost-only sandbox. If runtime availability blocks MB-003, record the blocker and stop; do not activate a second backlog item in the same handoff.
...
2. Read, in order:
   1. this file;
   2. `PROJECT_STATE.md`;
   3. the `MB-003` section of the authoritative Obsidian backlog;
...
- Update the authoritative Obsidian MB-003 item with closure evidence.
