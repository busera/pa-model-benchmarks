...
- Archived the strict-JSON `local_coding_breakpoint_benchmark.py` producer and historical tests under `scripts/_Archive/legacy_coding_breakpoint/`; no source or evidence was deleted.
- Added repository-owned `coding_workspace_benchmark.py` using workspace mode `W`: one host-selected file per model call, raw or ordinary fenced Python, no model-selected paths, no JSON source envelope, and no provider structured-output schema.
- Multi-file goals are generated sequentially with current workspace context. Compilation and sandboxed hidden tests run against actual files across up to three total workspace passes: the initial attempt plus at most two test-feedback corrections.
- Retained catalog, scheduling, resume, manifest, hidden-test, Seatbelt, provenance, and report regression coverage while replacing strict-JSON/M/A assumptions.
- Hardened generated-code file reads: runtime/system imports remain available, while home directories outside the workspace, mounted volumes, private data, applications, network mounts, global Library data, device data, and non-runtime `/opt` trees are denied. A direct regression requires controlled denial of `/etc/hosts`.
- Closed candidate-1 review findings: correction prompts reject symlinked workspace paths; hidden tests/config live outside the generated-code writable tree; pytest may write only invocation-owned temp/cache roots; complete hidden-harness, generated-file, and file-set hashes are verified after execution; integrity failures abort correction immediately; resume uses the same validated child run-ID boundary as new runs.
- Closed candidate-3 review findings: Hermes/Codex framing removal preserves fenced responses for single-fence validation; hidden-test PASS requires a positive collection census, matching zero-skip/zero-failure JUnit census, and an unguessable write-only nonce emitted after `pytest.main()` returns and bound to its return code; retained resume manifests bind `run_id` as an immutable provenance dimension.
- Historical strict-JSON scores are legacy transport-contract evidence and are not comparable to workspace-mode results.
- Pre-final live C0 smoke `20260730-gemma4-c0-format-neutral-smoke-completion` is diagnostic evidence bound to its manifest runner hash: normal fenced responses were written to `solution.py`, one correction fixed boolean handling, collection found exactly 7 tests, JUnit recorded 7 passed with zero failures/errors/skips, the write-only nonce completion marker was true, and all file-integrity checks were true. It did not execute the final staged runner.
- Full diagnostic run `20260730-gemma4-full-format-neutral-c0-c5` exposed two benchmark defects: whole-workspace compilation could let a stale later-file syntax error prevent correction from reaching that file, and completed failing pytest runs were incorrectly given an additional census-mismatch label. RED/GREEN regressions now compile only the just-generated file and authenticate completed pytest PASS or FAIL while preserving fail-closed rejection of skips and premature exits.
- Pre-final corrected single-repeat calibration `20260730-gemma4-full-format-neutral-c0-c5-corrected` completed all six cells. Gemma passed C0–C3 and failed C4–C5, for observed diagnostic ceiling C3 and breakpoint C4. C4's final generated code failed import during test collection (`callable[...]`); C5 completed all 8 tests with 3 failures involving prompt timeout return and migration-plan contracts. No provider, transport, timeout, sandbox, harness-integrity, generated-file-integrity, or file-set-integrity failure occurred. This run is bound to its manifest runner hash, did not execute the final staged runner, and does not authorize promotion.
- Final-review remediation requires affirmative Ollama completion, rejects token-cap and wrong-model envelopes, fails closed on Hermes maximum-iteration warnings, and marks Hermes route identity as diagnostic/unverified. Raw Hermes source is byte-preserved after positively identified session framing; unfenced reasoning frames fail closed. Resume now requires every retained cell's canonical `cell.json`, binds it to the exact schedule slot and artifact path, and rejects missing or conflicting terminal artifacts.
- Test classification is explicit: completed assertion failures use `HIDDEN_TEST_FAILURE`; module-level skip/empty collection uses `TEST_SKIPPED`; generated-code import/collection failure uses `GENERATED_CODE_COLLECTION_ERROR`; incomplete census remains `TEST_CENSUS_MISMATCH`.
- `kimi-k3:cloud` attempt `20260730-kimi-k3-cloud-full-format-neutral-c0-c5` completed zero model calls because Ollama Cloud returned HTTP 402 for every cell. It is provider/runtime diagnostic evidence only and is not a Kimi capability result.
- Final staged-source evidence is offline: `227 passed` plus compile, Ruff, diff, clean-export, and offline self-test gates. No live model run has executed the final staged runner.
